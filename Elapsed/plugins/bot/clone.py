...
DURATIONS = { "half_month": {"days": 15, "amount": 35}, "one_month": {"days": 30, "amount": 50}, "two_month": {"days": 60, "amount": 100}, }

@bot.on_message(filters.command("clone") & filters.private) async def start_clone_flow(client, message: Message): user_id = message.from_user.id name = message.from_user.first_name if len(message.command) > 1: string_token = message.command[1] payment_data = payments_collection.find_one({"user_id": user_id, "status": "approved"}) if not payment_data: await message.reply_text("You are not approved yet. Please pay and wait for approval before cloning.") return bots = list(cloned_bots_collection.find()) for bot_entry in bots: if bot_entry['string'] == string_token: await message.reply_text("This Assistant UserBot is already cloned. Use /startub if it's off.") return ai = Client( f"{name}", API_ID, API_HASH, session_string=string_token, plugins={"root": "Elapsed.plugins.userbot"}, ) try: await ai.start() await ai.join_chat(JOIN_CHAT) bot_user = await ai.get_me() details = { 'is_bot': False, 'user_id': user_id, 'name': bot_user.first_name, 'string': string_token, 'username': bot_user.username, 'cloned_at': datetime.utcnow() } cloned_bots_collection.insert_one(details) log_text = ( f"User Cloned UB:\n" f"Name: {bot_user.first_name}\n" f"User ID: {user_id}\n" f"Username: @{bot_user.username if bot_user.username else 'N/A'}\n" f"DC ID: {message.from_user.dc_id}\n" f"Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} (IST)" ) log_msg = await client.send_message(STORAGE_CHANNEL, log_text) payments_collection.update_one({"user_id": user_id}, {"$set": {"clone_log_id": log_msg.id}}) await message.reply_text(f"Cloned UB @{bot_user.username}. Use .start to activate. For help, type .help") except Exception as e: logging.exception(f"Error while cloning ub: {e}") await message.reply_text(f"Error during cloning: {e}") return
...
@bot.on_callback_query(filters.regex(r"pay_(half_month|one_month|two_month)")) async def handle_payment_selection(client, query: CallbackQuery): user_id = query.from_user.id duration_key = query.data.replace("pay_", "") duration_data = DURATIONS[duration_key]
...
@bot.on_message(filters.private & filters.photo) async def handle_payment_screenshot(client, message: Message): user_id = message.from_user.id pending = payments_collection.find_one({"user_id": user_id, "status": "pending"}) if not pending: await message.reply_text("No pending payment found. Start with /clone.") return
...
@bot.on_callback_query(filters.regex(r"(approve|decline)(\d+)")) async def handle_approval_decision(client, query: CallbackQuery): action, user_id = query.data.split("") user_id = int(user_id) user_data = payments_collection.find_one({"user_id": user_id}) if not user_data: await query.answer("User data not found or already processed.", show_alert=True) return
...
async def check_expired_access(): while True: now = datetime.utcnow() expired = payments_collection.find({"status": "approved", "expire_at": {"$lte": now}}) for user in expired: try: if "log_msg_id" in user: await bot.delete_messages(STORAGE_CHANNEL, message_ids=[user["log_msg_id"]]) except: pass payments_collection.delete_one({"user_id": user["user_id"]}) await asyncio.sleep(3600)
